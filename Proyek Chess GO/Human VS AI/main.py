...
def print_pos(pos, f=sys.stderr, owner_map=None):
    ...
    global pretty_board2
    pretty_board2 = ''.join(board.rstrip())
        ...
        for i in range(0,9):
            ...
            for j in range(0,11):
                if j==0 or j==1:
                     ...
                     if kondisi[counter] is not None:
                      satubaris.append(kondisi[counter])
                     else:
                        satubaris.append(0)
                counter=counter+1
            posisi.append(satubaris)
...
def execute2(cmd, prog, sim):
    ...
    while(cont):
        ...
        if err == True:
                 print ("------------")
                 print(makan[indexawal][indexkedua])
...
def check2(cmd, prog, sim, timeout):
    start = time.time()
    
    pool = ThreadPool(processes=1)
    a = pool.apply_async(execute2, (cmd,prog,sim,))
    c = None
    try:
        c = a.get(timeout=timeout)
    except TimeoutError:
    ...
    while True:
       ...
       # check timeout
        ...
        if index == 1:
            ...
            c = check2(cmd[index], prog[index], sim, timeout)
            
            if c is not None:
                # Not a pass
                sim = Simulation(pos=sim.pos.move2(c))
